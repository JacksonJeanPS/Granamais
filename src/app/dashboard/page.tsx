...
export default async function Page() {
  ...
  const [profileResult, accountsResult, transactionsResult, invoicesResult, goalsResult, debtsResult, debtPaymentsResult] = await Promise.all([
    ...
    supabase.from("debts").select("id,nome,credor,saldo_devedor,taxa_juros_mensal,valor_parcela,em_atraso,negativada,oferta_quitacao,data_vencimento,prioridade").eq("user_id", userId).in("status", ["aberta", "negociada"]),
    supabase.from("debt_payments").select("valor,data_pagamento").eq("user_id", userId).gte("data_pagamento", historyStart).order("data_pagamento"),
  ]);
  ...
  const currentDebtBalance = (debtsResult.data ?? []).reduce((sum, debt) => sum + Number(debt.saldo_devedor), 0);
  const debtTrend = buildDebtTrend(currentDebtBalance, debtPaymentsResult.data ?? [], now.year, now.month);
  const commitments = [
    ...(invoicesResult.data ?? []).map((invoice) => ({ id: `invoice-${invoice.id}`, name: invoice.cards?.nome ?? "Fatura do cartão", description: "Fatura", date: invoice.data_vencimento, value: Number(invoice.valor_total), color: invoice.cards?.cor ?? "#145C43" })),
    ...(debtsResult.data ?? []).filter((debt) => debt.data_vencimento && debt.data_vencimento >= monthStart && debt.data_vencimento <= ninetyDays).map((debt) => ({ id: `debt-${debt.id}`, name: debt.nome, description: debt.em_atraso ? "Dívida em atraso" : "Parcela de dívida", date: debt.data_vencimento!, value: Number(debt.valor_parcela ?? debt.saldo_devedor), color: debt.em_atraso ? "#B42318" : "#8B5E18" })),
  ...
  return <div className="mx-auto max-w-[1400px] space-y-7">
    ...
    <section className="grid gap-6 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,.8fr)]"><Card><CardHeader className="flex-row items-start justify-between"><div><CardTitle>Fluxo de caixa</CardTitle><p className="mt-1 text-sm text-muted-foreground">Receitas e despesas dos últimos 6 meses</p></div><div className="hidden gap-3 text-xs sm:flex"><Legend color="bg-positive" label="Receitas" /><Legend color="bg-negative" label="Despesas" /></div></CardHeader><CardContent><CashFlowChart data={buildChart(transactions, now.year, now.month)} /></CardContent></Card><Card><CardHeader><CardTitle className="text-xl">Próximos compromissos</CardTitle></CardHeader><CardContent className="space-y-3">{commitments.length ? commitments.map((item) => <div key={item.id} className="flex items-center gap-3 rounded-2xl bg-subtle p-4"><span className="rounded-xl bg-card p-2.5"><CreditCard className="size-4" style={{ color: item.color }} /></span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{item.name}</p><p className="text-xs text-muted-foreground">{item.description} · vence {formatShortDate(item.date)}</p></div><p className="text-sm font-bold">{formatCurrency(item.value)}</p></div>) : <Empty icon={CreditCard} text="Nenhuma fatura ou dívida vence nos próximos 90 dias." />}</CardContent></Card></section>
    {currentDebtBalance > 0 ? <Card><CardHeader className="flex-row items-start justify-between"><div><CardTitle>Evolução das dívidas</CardTitle><p className="mt-1 text-sm text-muted-foreground">Saldo reconstruído pelos pagamentos registrados nos últimos 6 meses</p></div><Button asChild variant="ghost" size="sm"><Link href="/dividas">Ver plano</Link></Button></CardHeader><CardContent><DebtBalanceChart data={debtTrend} /></CardContent></Card> : null}
...
function buildDebtTrend(currentBalance: number, payments: { valor: number; data_pagamento: string }[], year: number, month: number) { const months = Array.from({ length: 6 }, (_, index) => { const date = new Date(Date.UTC(year, month - 6 + index, 1)); return { key: date.toISOString().slice(0, 7), month: monthLabels[date.getUTCMonth()] }; }); let balance = currentBalance + payments.reduce((sum, payment) => sum + Number(payment.valor), 0); return months.map((item) => { balance -= payments.filter((payment) => payment.data_pagamento.startsWith(item.key)).reduce((sum, payment) => sum + Number(payment.valor), 0); return { month: item.month, balance: Math.max(0, balance) }; }); }
